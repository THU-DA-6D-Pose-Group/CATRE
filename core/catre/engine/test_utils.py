    ...
    for progress, result in tqdm(enumerate(final_results)):
        ...
        for cls_id in range(1, num_classes):
            ...
            RT_overlaps = compute_RT_overlaps(
                cls_gt_class_ids, cls_gt_RTs, cls_gt_handle_visibility, cls_pred_class_ids, cls_pred_RTs, synset_names
            )
            pose_cls_gt_match, pose_cls_pred_match = compute_match_from_degree_cm(
                RT_overlaps, cls_pred_class_ids, cls_gt_class_ids, degree_thres_list, shift_thres_list
            )
        ...
        for j, shift_thres in enumerate(shift_thres_list):
            for cls_id in range(1, num_classes):
                ...
                pose_aps[cls_id, i, j] = compute_ap_from_matches_scores(
                    cls_pose_pred_matches_all, cls_pose_pred_scores_all, cls_pose_gt_matches_all
                )
...
def evaluate(path):
    ...
    print("3D IoU at 75: {:.1f}".format(iou_3d_aps[-1, iou_thres_list.index(0.75)] * 100))

    print("5 degree, 2cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(2)] * 100))
    print("5 degree, 5cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(5), shift_thres_list.index(5)] * 100))

    print("10 degree, 2cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(2)] * 100))
    print("10 degree, 5cm: {:.1f}".format(pose_aps[-1, degree_thres_list.index(10), shift_thres_list.index(5)] * 100))


if __name__ == "__main__":
    path = "/data/lxy/deepim-laf/datasets/NOCS/dualposenet_results/REAL275_results_mvmug.pkl"
    evaluate(path)
