...
spd_pose_dir = osp.join(PROJ_ROOT, "datasets/NOCS/deformnet_eval/eval_camera")  # in bbnc11
spd_seg_dir = osp.join(PROJ_ROOT, "datasets/NOCS/deformnet_eval/mrcnn_results/val")

# our format
init_pose_dir = osp.join(data_root, "test_init_poses")
mmcv.mkdir_or_exist(init_pose_dir)
init_pose_path = osp.join(init_pose_dir, "init_pose_spd_nocs_cmra.json")
...
if __name__ == "__main__":
    ...
    if CACHED:
        ...
        for idx, spd_pose_path in enumerate(tqdm(spd_pose_paths)):
            ...
            seg_path = osp.join(spd_seg_dir, f"results_val_{scene_id}_{im_id}.pkl")
