...
class NOCS_Dataset:
    ...
    def __init__(self, data_cfg):
        ...
        # NOTE: only real, no camera
        self.scales_dict = ref.nocs.get_scales_dict()
        ...
        self.cam = ref.nocs.intrinsics
        ##################################################

        self.catid2names = {cat_id: obj_name for cat_id, obj_name in ref.nocs.id2obj.items() if obj_name in self.objs}
    ...
    def _load_from_idx_file(self, idx_file, image_root):
        ...
        for split_scene_im_id in tqdm(split_scene_im_ids):
            ...
            for idx, inst_id in enumerate(gt_dict["instance_ids"]):
                ...
                abs_scale = self.scales_dict[inst_name]
...
update_cfgs = dict(
    nocs_train_real=dict(
        objs=ref.nocs.objects,
        dataset_root=osp.join(DATASETS_ROOT, "NOCS/REAL/real_train"),
        model_path=osp.join(DATASETS_ROOT, "NOCS/obj_models/real_train_spd.pkl"),
        ann_files=[
            osp.join(
                DATASETS_ROOT,
                "NOCS/REAL/image_set/train_list.txt",
            )
        ],
        img_type="real",
        image_prefixes=[osp.join(DATASETS_ROOT, "NOCS/REAL/")],
    ),
    nocs_test_real=dict(
        objs=ref.nocs.objects,
        dataset_root=osp.join(DATASETS_ROOT, "NOCS/REAL/real_test"),
        model_path=osp.join(
            DATASETS_ROOT, "NOCS/obj_models/real_test_spd.pkl"
        ),  # NOTE: this is not allowed to use during inference!
        ann_files=[
            osp.join(
                DATASETS_ROOT,
                "NOCS/REAL/image_set/test_list.txt",
            )
        ],
        img_type="real",
        image_prefixes=[osp.join(DATASETS_ROOT, "NOCS/REAL/")],
...
for obj in ref.nocs.objects:
    for split in ["train_real", "test_real"]:
        mode = split.replace("_real", "")  # train/test/val
        ...
        if name not in SPLITS_NOCS:
            update_cfgs[name] = dict(
                objs=[obj],
                dataset_root=osp.join(DATASETS_ROOT, f"NOCS/REAL/real_{mode}"),
                model_path=osp.join(DATASETS_ROOT, f"NOCS/obj_models/real_{mode}.pkl"),
                ann_files=[
                    osp.join(
                        DATASETS_ROOT,
                        f"NOCS/REAL/image_set/{obj}_real_{mode}.txt",
                    )
                ],
                image_prefixes=[osp.join(DATASETS_ROOT, f"NOCS/REAL/")],
                img_type="real",
            )
...
def register_with_name_cfg(name, data_cfg=None):
    ...
    if name in SPLITS_NOCS:
        ...
        used_cfg = data_cfg
    DatasetCatalog.register(name, NOCS_Dataset(used_cfg))
...
if __name__ == "__main__":
    ...
    from core.utils.cat_data_utils import get_bbox_from_scale
